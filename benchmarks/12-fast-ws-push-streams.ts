/**
 * Benchmark: Push Stream Performance (with Fast WebStreams)
 *
 * Mirrors 02-push-streams.ts but adds Vercel's experimental-fast-webstreams
...
async function runBenchmarks(): Promise<FourWayComparison[]> {
  ...
  console.log('Running: Concurrent push (medium chunks)...');
  {
    ...
    comparisons.push(createFourWayComparison('Concurrent push (4KB x 1000)', newStreamResult, fastWebStreamResult, webStreamResult, nodeStreamResult));
  }
  ...
  console.log('Running: Many small writes...');
  {
    ...
    comparisons.push(createFourWayComparison('Many small writes (64B x 10000)', newStreamResult, fastWebStreamResult, webStreamResult, nodeStreamResult));
  }
  ...
  console.log('Running: Batch writes...');
  {
    ...
    comparisons.push(createFourWayComparison('Batch writes (512B x 20 x 200)', newStreamResult, fastWebStreamResult, webStreamResult, nodeStreamResult));
  }
  ...
  console.log('Running: Push + async iteration...');
  {
    ...
    comparisons.push(createFourWayComparison('Push + async iter (2KB x 1000)', newStreamResult, fastWebStreamResult, webStreamResult, nodeStreamResult));
  }
...
// Main
console.log('Benchmark: Push Stream Performance (with Fast WebStreams)');
