/**
 * Benchmark: Pipeline Performance — effect/Stream
 *
 * Mirrors 04-pipelines.ts, adding effect/Stream as a fourth implementation.
 * Shows how effect/Stream compares in full source → transform → sink flows.
 *
 * effect/Stream patterns used:
 *   Simple pipeline:    Stream.fromIterable + Stream.runFold
 *   XOR pipeline:       Stream.map (CPU-bound transform) + Stream.runFold
 *   Multi-stage:        Three chained Stream.map calls + Stream.runFold
 *   High-frequency:     Many tiny chunks through Stream.runFold
 */
...
// Main
console.log('Benchmark: Pipeline Performance — effect/Stream');
console.log('Comparing: New Streams, Web Streams, Node.js Streams, effect/Stream');
console.log('effect/Stream: Stream.fromIterable + Stream.map + Stream.runFold');
