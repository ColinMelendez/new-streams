/**
 * Benchmark: Transform Performance — effect/Stream v4 beta
 *
 * Mirrors 19-effect-transforms.ts using effect v4 (installed as "effect-v4").
 ...
 *     replaces Stream.mapConcatChunk(c => Chunk.make(c, c)) (removed in v4)
 *   - No Chunk import needed (runCollect returns Array<A>)
 */
...
async function runBenchmarks(): Promise<EffectComparison[]> {
  ...
  // elements into an Array before mapEffect fires, matching the batch-level
  ...
  console.log('Running: Async transform (grouped)...');
  {
    ...
    // Effect v4: Stream.grouped(n) collects n elements into an Array, then
    ...
    comparisons.push(createEffectComparison(
      'Async transform grouped (8KB x 300)',
      newStreamResult, webStreamResult, nodeStreamResult, effectStreamResult
    ));
  }
...
// Main
console.log('Benchmark: Transform Performance — effect/Stream v4 beta');
console.log('Comparing: New Streams, Web Streams, Node.js Streams, effect/Stream v4');
console.log('effect/Stream v4: Stream.map, Stream.map+flattenArray, Stream.mapEffect');
